...
# SOQL queries for direct threat detection event store tables
# Each event store has different field schemas
# Include Id for tie-breaking on same EventDate timestamps
SOQL_THREAT_DETECTION = {
    "ApiAnomaly": (
        "SELECT Id, Score, UserId, EventDate, SecurityEventData, Summary "
        "FROM ApiAnomalyEventStore "
        "WHERE EventDate >= {pointer}"
    ),
    "SessionHijacking": (
        "SELECT Id, Score, UserId, EventDate, SecurityEventData, Summary "
    ...
    "CredentialStuffing": (
        "SELECT Id, Score, UserId, EventDate, LoginUrl, LoginType, Summary "
    ...
    "BulkApiResult": (
        "SELECT Id, UserId, EventDate, ApiType, ApiVersion, RequestId, "
        "ResponseSize, ResponseTime, StatusCode, Summary "
        "FROM BulkApiResultEventStore "
        "WHERE EventDate >= {pointer}"
    ),
    "GuestUserAnomaly": (
        "SELECT Id, Score, UserId, EventDate, SecurityEventData, Summary "
        "FROM GuestUserAnomalyEventStore "
        "WHERE EventDate >= {pointer}"
    ),
    "LoginAnomaly": (
        "SELECT Id, Score, UserId, EventDate, LoginUrl, LoginType, Summary "
    ...
    "PermissionSet": (
        "SELECT Id, UserId, EventDate, PermissionSetId, PermissionSetName, "
        "PermissionSetLabel, Action, Summary "
        "FROM PermissionSetEventStore "
        "WHERE EventDate >= {pointer}"
    )
}
...
class Connector(BaseConnector):
    ...
    def get_oauth_access_token(self) -> Tuple[str, str]:
        ...
        try:
            ...
            if not instance_url:
                raise RequestFailedException("No instance URL received from Salesforce; set configuration.instance_url")
    ...
    def _check_shield_availability(self, client: Salesforce) -> None:
        ...
        try:
            ...
            self.logger.debug("Salesforce Shield Event Monitoring is available", extra=self.log_context)
        except SalesforceError as err:
            error_code = getattr(err, 'error_code', None)
            if error_code == 'INVALID_TYPE':
                raise RequestFailedException(
                    ...
                    f"Error: {err}\n\n"
                    f"Please verify:\n"
                    f"1. Salesforce Shield is purchased and enabled in your org\n"
                    f"2. Event Monitoring is activated in Setup > Event Monitoring Settings\n"
                    f"3. Your user has 'View Event Monitoring Data' permission\n"
                    f"4. Your org has the necessary Shield licenses\n\n"
                    f"Contact your Salesforce administrator to enable Shield Event Monitoring."
                )
            else:
    ...
    def collect(self):  # noqa: C901
        ...
        try:
            ...
            soql_query = SOQL_THREAT_DETECTION[self.operation].format(
            ...
            if records.get('totalSize', 0) == 0:
                ...
                # Rebuild the query with a new pointer timestamp instead of string replacement
                broader_pointer = (now - timedelta(days=30)).strftime("%Y-%m-%dT00:00:00.00Z")
                broader_query = SOQL_THREAT_DETECTION[self.operation].format(
                    pointer=broader_pointer
                )
            ...
            if error_code == 'INVALID_TYPE':
                ...
                raise RequestFailedException(
                    f"Salesforce Shield licensing or permissions issue detected. "
                    f"Error: {error_message}\n\n"
                    f"Please verify:\n"
                    f"1. Salesforce Shield is enabled in your org\n"
                    f"2. Your user has 'View Threat Detection Events' permission\n"
                    f"3. The event store '{self.operation}EventStore' is available\n"
                    f"4. Your profile/permission set includes Shield Event Monitoring access\n\n"
                    f"Contact your Salesforce administrator to enable Shield Event Monitoring."
                )
            elif error_code == 'INVALID_FIELD':
                # Field-level permissions issue
                raise RequestFailedException(
                    f"Field-level permissions issue detected. "
                    f"Error: {error_message}\n\n"
                    f"Please verify:\n"
                    f"1. Your user has field-level access to the requested fields\n"
                    f"2. The event store schema is correct for your Salesforce version\n"
                    f"3. Shield Event Monitoring is properly configured\n\n"
                    f"Contact your Salesforce administrator to check field permissions."
                )
            else:
                # Other Salesforce errors
                raise RequestFailedException(
                    f"Salesforce API error ({error_code}): {error_message}"
                )
        except Exception as err:
            raise RequestFailedException(
        ...
        for i, record in enumerate(records.get("records", [])):
            ...
            # Add operation-specific fields based on EventStore schema
            if self.operation in ["CredentialStuffing", "LoginAnomaly"]:
                # Login-related events have LoginUrl and LoginType
                entry["LoginUrl"] = record.get("LoginUrl")
                entry["LoginType"] = record.get("LoginType")
            elif self.operation == "BulkApiResult":
                # Bulk API events have API-specific fields
                entry["ApiType"] = record.get("ApiType")
                entry["ApiVersion"] = record.get("ApiVersion")
                entry["RequestId"] = record.get("RequestId")
                entry["ResponseSize"] = record.get("ResponseSize")
                entry["ResponseTime"] = record.get("ResponseTime")
                entry["StatusCode"] = record.get("StatusCode")
            elif self.operation == "PermissionSet":
                # Permission Set events have permission-specific fields
                entry["PermissionSetId"] = record.get("PermissionSetId")
                entry["PermissionSetName"] = record.get("PermissionSetName")
                entry["PermissionSetLabel"] = record.get("PermissionSetLabel")
                entry["Action"] = record.get("Action")
            else:
                # Other events (ApiAnomaly, SessionHijacking, ReportAnomaly, GuestUserAnomaly)
                # have SecurityEventData
                entry["SecurityEventData"] = record.get("SecurityEventData")
                    ...
                    if entry.get("EventDate") and entry.get("Id"):
                        try:
                            ...
                            if (latest_time is None or 
                                entry_time > latest_time or 
                                (entry_time == latest_time and entry_id > latest_id)):
