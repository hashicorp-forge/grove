...
"""Tests for the Salesforce Field Audit Trail connector."""
...
from grove.connectors.sf.field_audit_trail import Connector
from grove.exceptions import ConfigurationException, RequestFailedException


class TestSFFieldAuditTrailConnector:
    """Tests for the Salesforce Field Audit Trail connector."""
    ...
    def test_missing_object_type_raises_exception(self):
        """Test that missing object_type raises an exception."""
        config = MagicMock()
    ...
    def test_collect_oauth_success(self, mock_session, mock_salesforce, connector):
            ...
            "records": [
                {
                    ...
                    "FieldHistoryType": "Account",
                    "Field": "Name",
                    "OldValue": "Old Account Name",
        ...
        assert "FieldHistoryArchive" in query_args
        assert "FieldHistoryType = 'Account'" in query_args
        ...
        assert saved_entries[0]["_grove_operation"] == "FieldAuditTrail"
        assert saved_entries[0]["object_type"] == "Account"
        assert saved_entries[0]["Field"] == "Name"
        assert saved_entries[0]["OldValue"] == "Old Account Name"
    ...
    def test_collect_invalid_type_error(self, mock_session, mock_salesforce, connector):
        ...
        mock_client = MagicMock()
        mock_client.query_all.side_effect = SalesforceError("INVALID_TYPE: sObject type 'FieldHistoryArchive' is not supported", 400, "test", "test")
        mock_salesforce.return_value = mock_client
        ...
        assert "Shield Field Audit Trail licensing" in str(exc_info.value)
    ...
    def test_collect_invalid_field_error(self, mock_session, mock_salesforce, connector):
        ...
        # Mock Salesforce client with INVALID_FIELD error
        mock_client = MagicMock()
        mock_client.query_all.side_effect = SalesforceError("INVALID_FIELD: No such column 'ParentId'", 400, "test", "test")
        mock_salesforce.return_value = mock_client
        ...
        assert "Field-level permissions issue" in str(exc_info.value)
    ...
    def test_parse_salesforce_timestamp(self):
        ...
        from grove.connectors.sf.field_audit_trail import parse_salesforce_timestamp
    ...
    def test_supported_object_types_warning(self):
        ...
        config.client_secret = "test_client_secret"
        config.instance_url = "https://testorg.my.salesforce.com"
        config.object_type = "CustomObject__c"  # Not in supported list
        config.start_date = "2024-01-01T00:00:00.000Z"
        config.max_retries = 3
        config.retry_delay = 1
    ...
    def test_field_audit_trail_availability_check(self, mock_session, mock_salesforce, connector):
        ...
        # Mock Salesforce client with availability check failure
        mock_client = MagicMock()
        mock_client.query.side_effect = SalesforceError("INVALID_TYPE: sObject type 'FieldHistoryArchive' is not supported", 400, "test", "test")
        mock_salesforce.return_value = mock_client
        ...
        assert "Field Audit Trail is not available" in str(exc_info.value)
        assert "Shield Field Audit Trail license" in str(exc_info.value)
